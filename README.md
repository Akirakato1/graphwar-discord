...
- Current checkpoint: server-side normal-function parsing, authoritative shot simulation with collision resolution, destructible terrain with circular crater explosions, match modes, deterministic map generators, an authoritative match controller, and local WebSocket room orchestration.
...
- Current: added local WebSocket rooms for validated client commands, lobby snapshots, match starts, shot resolution broadcasts, safe command rejection, and room integration tests.
