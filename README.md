...
- `apps/map-maker`: local Electron + React map editor with Canvas-style SVG editing, mouse-wheel zoom-at-cursor, middle mouse or Space+drag panning, 5-unit major and subtle zoom-gated 1-unit minor grid lines, rectangle/triangle/circle terrain tools, a pen tool with right-click undo and close-to-first-node completion, draggable terrain and spawn points, transform handles for terrain scaling, team A/team B spawn assignment, a 10-spawn helper, and native export to shared-validated `.graphwar-map.json`.
...
The map maker exports `.graphwar-map.json` files through a native save dialog. Use the mouse wheel to zoom around the cursor, middle mouse or Space+drag to pan, and the 5-unit major grid plus zoom-gated subtle 1-unit minor grid for placement. Import the exported file from the Activity's `Custom Maps` menu, then select it from the Create Lobby `Map` field.
