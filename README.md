...
- Added leader-confirmed open-lobby cancellation over the guild WebSocket protocol, returning all connected setup clients to the main menu and hiding the cancelled lobby from guild listings, plus terrain-aware default-map spawn nudging so generated spawns do not start inside terrain.
- Added in-game forfeits through a warning-confirmed `FF` HUD button, with server-authoritative dead-state resolution, winner checks, and local return-to-menu behavior for the forfeiting player; changed the derivative snippet to first-derivative `diff(x,1,)` with cursor placement in the body slot.
- Added per-lobby turn timers with server-stamped turn deadlines, refresh-friendly countdown state, expired-shot rejection and auto-advance, plus keyboard/keypad/hybrid function input modes with a rendered math preview, cursor/delete keypad controls, and removal of the `x^2`, Wave, and factorial buttons.
